...
// T10/R2 re-base port (2026-06-11): claw wiring (runClaw + writeAssertionResult,
// deleted in #010) replaced with the post-#5 runAgent convention. The v4 spec —
// VERSION_SEED, FILES, all generation code, PROMPT, VERIFY_JS — is byte-identical
// to the last claw-era version (git df50d21~1). Only the harness wiring changed:
//   - reset/seed/post-script/registry-sidecar now run through runAgent;
//   - the 30 nested fixture files are written by a pluggable runner wrapper
//     (runAgent.seedFiles is flat-only; the wrapper writes them into /workspace
//     after runAgent's reset+seed, then delegates to the CONFIG-selected runner);
//   - pass oracle is the central #001 workspace oracle (postScript verify.js
//     exit 0); the claw-era `r.code === 0` gate is telemetry now, lab-wide.

import { describe, it } from 'node:test';
import crypto from 'node:crypto';
import fs from 'node:fs';
import path from 'node:path';
import { runAgent, selectRunner } from '../../lib/runAgent.js';
...
// Pluggable-runner wrapper (port shim): runAgent.seedFiles writes flat names
// only (plain writeFileSync, no nested mkdir), so the 30 subdir fixtures are
// written here — after runAgent's reset + seed (verify.js) and immediately
// before the agent starts — then the call delegates to the same CONFIG-selected
// runner defaultRunner would have used (selectRunner, lib/runAgent.js). The
// workspace the model sees is byte-identical to the claw-era beforeEach seed.
function seededNeedleRunner(opts) {
  const subdirs = new Set(FILES.map((f) => path.dirname(f)));
  for (const d of subdirs) {
    fs.mkdirSync(path.join(workspace.WORKSPACE, d), { recursive: true });
  }
  for (const f of FILES) {
    fs.writeFileSync(path.join(workspace.WORKSPACE, f), generateFile(f));
  }
  return selectRunner()(opts);
}

describe(`needle-haystack: 30-file NIAH apply-the-needle (tier=${TIER_LABEL})`, () => {
  it('agent locates REGION_KEY and writes solve.js', { timeout: TIMEOUT }, async (t) => {
    ...
    // Oracle equivalence note: the claw-era test asserted exit-0 + solve.js
    // exists + verify.js exit 0. Under #001 the agent exit code is telemetry
    // lab-wide, and solve.js-exists is implied by verify.js (it imports
    // ./solve.js). No unchanged() assert is added — the original oracle did
    // not check verify.js integrity, and the port must not be stricter.
    await runAgent({
      prompt:        PROMPT,
      seedFiles:     { 'verify.js': VERIFY_JS },
      postScript:    'verify.js',
      clawTimeoutMs: TIMEOUT,
      testId:        'needle-haystack',
      runner:        seededNeedleRunner,
      t,
    });
  });
});
